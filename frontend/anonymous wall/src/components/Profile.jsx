...
const Profile = () => {
  ...
  const followAndUnfollowHandler = async () => {
    if (user.following.includes(id)) {
      try {
        await axios.put(`${USER_API}/unfollow/${id}`, { id: user?._id }, {
      ...
      try {
        await axios.put(`${USER_API}/follow/${id}`, { id: user?._id }, {
          withCredentials: true,
        });
  ...
  const userTweets = tweets?.filter((tweet) => tweet?.userId?._id === profile?._id);

  return (
    <div className='w-[50%] border-l border-r border-gray-200 h-screen flex flex-col'>
      ...
      <div className='flex-shrink-0'>
        <div className='flex items-center py-2'>
          ...
          <div className='ml-6'>
            ...
            <p className='text-gray-500 text-sm'>Post {userTweets?.length}</p>
          </div>
        </div>
        ...
        <div className='text-right m-4'>
          {
            profile?._id === user?._id ? (
              ...
              <button onClick={followAndUnfollowHandler} className='px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 font-bold'>
